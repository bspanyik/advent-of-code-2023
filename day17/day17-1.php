...
const MAX_STEPS_IN_A_DIRECTION = 3;
...
while (!$priorityQueue->isEmpty()) {
    ...
    if ($row === $mapHeight - 1 && $col === $mapWidth - 1) {
        $time = microtime(true) - $begins;
        echo sprintf('%d in %f seconds.' . PHP_EOL, $heatLoss, $time);
        exit;
    }

    // emulating a Set with array-keys (much faster than in_array)
